...
namespace BilardGame
{
    class Filling
    {
        class Node
        {
            ...
            public Node(Point3D p1, Point3D p2, Node _next = null)
            {
        ...
        public Filling(int size)
        {
            bucketTable = new Node[size + 1];
        }
        void FillTable(IEnumerable<Point3D> Polygon)
        {
            Ymax = 0; Ymin = int.MaxValue;
            Point3D prevPoint = Polygon.LastOrDefault();
        ...
        public void Draw(IEnumerable<Point3D> Polygon, Action<int, int> Fill)
        {
            scanLine = null;
            // sort edges
            FillTable(Polygon);
